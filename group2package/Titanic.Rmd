...
logistic_regression_model_1(dataset)
```

```{r}
prob_survival(dataset, 1,1,15)
```


```{r}
survival_stats(dataset)
```

```{r}
survival_plots(dataset, 20)
```

```{r}
plots_by_class(dataset, 20)
```

```{r}
plots_by_sex(dataset, 20)
```

```{r}
compare_model_w_reality(dataset, 0.7)
```

```{r}
compare_probs_survival(dataset)
```

```{r}
percent_survival_correct(dataset)
```
...
prob_survival <- function(pclass, sex_binary, age){ #need to have the logistic_regression_model function as an argument here
  ...
  log_odds_survival <- (log_reg_output_reorg$estimate[1,2])+(log_reg_output_reorg$estimate[2,2])+(log_reg_output$*sex_binary)+(log_reg_output$beta3*age)
